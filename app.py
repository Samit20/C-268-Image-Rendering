...
def oil_effect(decode_array_to_img): #267
    bgr = cv2.cvtColor(decode_array_to_img, cv2.COLOR_RGB2BGR)
    oil_effect_img = cv2.xphoto.oilPainting(bgr, 7, 1)
    rgb = cv2.cvtColor(oil_effect_img, cv2.COLOR_BGR2RGBA)
    status, output_img = cv2.imencode('.PNG', rgb)
